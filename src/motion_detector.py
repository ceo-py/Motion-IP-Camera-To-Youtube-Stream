...
class MotionDetector:
    def __init__(self, camera_name, stream_url):
        ...
        self.motion_stream_url = stream_url[:-1] + "1"
    ...
    def get_frame(self):
        ...
        if not cap.isOpened():
            print_message(f"[{self.camera_name}] Error: Could not open motion stream: {self.motion_stream_url}")
            return None
        for _ in range(5):
            cap.grab()
        ret, frame = cap.read()
        cap.release()
        if not ret:
            print_message(f"[{self.camera_name}] Error: Could not retrieve frame from stream.")
    ...
    def check_motion(self):
        ...
        if frame is not None and not self.confirmed_connection:
            print_message(f"[{self.camera_name}] Connection confirmed. Monitoring for motion...")
    ...
    def start_stream(self):
        try:
            ...
            print_message(f"Error starting stream for {self.camera_name}: {e}")

    def stop_stream(self):
        try:
            subprocess.run([sys.executable, STOP_STREAM_SCRIPT, self.camera_name])
        except Exception as e:
            print_message(f"Error stopping stream for {self.camera_name}: {e}")
